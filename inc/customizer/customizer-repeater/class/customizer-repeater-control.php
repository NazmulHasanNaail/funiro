...
class Funiro_Repeater extends WP_Customize_Control {
	...
	public function __construct( $manager, $id, $args = array() ) {
		...
		if ( file_exists( FUNIRO_TEMPLATE_DIR . '/inc/customizer/customizer-repeater/inc/icons.php' ) ) {
	...
	public function enqueue() {
		wp_enqueue_style( 'font-awesome', FUNIRO_TEMPLATE_DIR_URI . '/assets/css/fontawesome-all.min.css', array(), 999 );

		wp_enqueue_style( 'funiro_customizer-repeater-admin-stylesheet', FUNIRO_TEMPLATE_DIR_URI . '/inc/customizer/customizer-repeater/css/admin-style.css', array(), 999 );

		wp_enqueue_style( 'wp-color-picker' );

		wp_enqueue_script( 'funiro_customizer-repeater-script', FUNIRO_TEMPLATE_DIR_URI. '/inc/customizer/customizer-repeater/js/customizer_repeater.js', array('jquery', 'jquery-ui-draggable', 'wp-color-picker' ), 999, true  );

    	wp_enqueue_style( 'funiro-admin-style',  FUNIRO_TEMPLATE_DIR_URI. '/assets/css/funiro-admin.css');    


		wp_enqueue_script( 'funiro_customizer-repeater-fontawesome-iconpicker', FUNIRO_TEMPLATE_DIR_URI . '/inc/customizer/customizer-repeater/js/fontawesome-iconpicker.js', array( 'jquery' ), 999, true );

		wp_enqueue_style( 'funiro_customizer-repeater-fontawesome-iconpicker-script', FUNIRO_TEMPLATE_DIR_URI . '/inc/customizer/customizer-repeater/css/fontawesome-iconpicker.min.css', array(), 999 );

		wp_enqueue_script('funiro-admin-script', get_template_directory_uri() . '/assets/js/funiro-admin-script.js', array( 'jquery', 'jquery-ui-sortable', 'customize-controls' ) );
	...
	private function iterate_array($array = array()){
		...
		if(!empty($array)){
			...
			$funiro_del_btn_id=$this->boxtitle;

			global $funiro_limit;
			global $type_with_id;
			echo sprintf("<input type='hidden' value='$exist_ser' id='exist_funiro_$funiro_del_btn_id'/>");
			foreach($array as $icon){
			if($it<4)
			{
			$funiro_limit="funiro_limit";
			$type_with_id='';
			}
			else
			{
			$funiro_limit="funiro_overlimit";
			$type_with_id=$funiro_del_btn_id."_".$it;
			}
				...
				<div class="customizer-repeater-general-control-repeater-container customizer-repeater-draggable">
					...
					<div class="customizer-repeater-box-content-hidden">
						...
						if($this->customizer_repeater_checkbox_control == true){
							$this->testimonila_check($open_new_tab, $funiro_limit, $type_with_id);
						...
						if($this->customizer_repeater_image_control == true && $this->customizer_repeater_icon_control == true) {
							$this->icon_type_choice(  $choice,$funiro_limit );
						}
						if($this->customizer_repeater_image_control == true){
							$this->image_control($image_url, $choice, $funiro_limit, $it+1, $funiro_del_btn_id);
						}
						if($this->customizer_repeater_icon_control == true){
							$this->icon_picker_control($icon_value, $choice, $funiro_limit);
						}
						...
						if($this->customizer_repeater_repeater_control==true){
							$this->repeater_control($repeater, $funiro_limit, $type_with_id);
						} ?>
	...
	private function icon_picker_control($value = '', $class='', $show = ''){ ?>
		<div class="social-repeater-general-control-icon" <?php if( $show === 'customizer_repeater_image' || $show === 'customizer_repeater_none' ) { echo 'style="display:none;"'; } ?>>
			...
			<div class="input-group icp-container">
				<input data-placement="bottomRight" class="<?php if($class=="funiro_limit") { echo 'icp icp-auto';} else { echo 'innofit-overlimit-icon-picker icp icp-auto';}?>" value="<?php if(!empty($value)) { echo esc_attr( $value );} ?>" type="text">
		...
		if ( ( count( $social_repeater ) == 1 && '' === $social_repeater[0] ) || empty( $social_repeater ) ) { ?>
			...
			<div class="customizer-repeater-social-repeater">
				...
				foreach ( $social_repeater as $social_icon ) {
					...
					<div class="customizer-repeater-social-repeater-container">
						<div class="customizer-repeater-rc input-group icp-container">
							<input data-placement="bottomRight" class="icp icp-auto team_data_<?php echo esc_attr($funiro_limit);?> <?php echo esc_attr($type_with_id);?>" value="<?php if( !empty($social_icon['icon']) ) { echo esc_attr( $social_icon['icon'] ); } ?>" type="text">
